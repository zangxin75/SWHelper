Sub M24Complete()
  ...
  If Not tF Is Nothing Then
    ...
    For i = 0 To UBound(vE): If i >= 6 Then Exit For
  ...
  If Not bF Is Nothing Then
    ...
    m.ClearSelection2 True
    For i = 0 To UBound(vE2): If i >= 6 Then Exit For
  ...
  ' === Swept Cut ===
  m.ClearSelection2 True
  b = m.Extension.SelectByID2(m.FeatureManager.GetFeatures(False).Item("螺旋线/涡状线1").Name, "REFERENCECURVES", 0, 0, 0, False, 1, Nothing, 0)
  Dim fm2 As Object: Set fm2 = m.FeatureManager
  Dim fts As Variant: fts = fm2.GetFeatures(False)
  Dim skName As String
  For i = 0 To UBound(fts)
    If fts(i).GetTypeName = "ProfileFeature" Then skName = fts(i).Name
  Next
  b = m.Extension.SelectByID2(skName, "SKETCH", 0, 0, 0, True, 4, Nothing, 0)
